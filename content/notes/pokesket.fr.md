---
title: "Pokesket"
date: "2025-06-15"
slug: "pokesket"
description: "Un petit projet de cours qui s'apparente plus à un projet entre pote finalement."
tags: ["unity", "methil", "ecole"]
---


# Un jeu vidéo, encore ?

Oui exactement, encore, j'aime plutôt pas mal les jeux vidéos et en entrant dans mon école, j'avais bon espoir que j'allais en faire (et j'en ai fait :3), mais ce sont souvent des projets un peu rush avec des deadlines assez serrées et des autres choses à faire en même temps, bref, pas toujours le temps de bien faire les choses et de s'amuser.

# Donc le départ

Mais pour Pokésket, c'est différent, pour le cours de Unity avec Colin Peyrat dans le cadre de ma 3ème année de bachelor, je me suis dit que j'allais plutôt prendre le truc vraiment au sérieux et que j'allais vraiment tryhard en me mettant avec mon pote **Emmanuel Moulin** ([GitHub](https://github.com/Kibishi47)). 
Et donc, fallait une idée, faire un petit jeu bête, qui a pas vraiment de sens, et je jouais un peu à NBA pendant cette période donc je me suis dit, on est pas designers, on est pas des artistes, on est mauvais en 3D, mais on est bons en code, alors faisons un jeu de sport, mais en 2D, avec des sprites, et un gameplay simple mais efficace. Donc on a fait Pokésket, un jeu de basket avec des pokémons. (Ce qui est donc parfait car des sprites existent déjà pour Pokémon, et que c'est donc plus simple pour nous.)

Un petit jeu 1V1 sur le thème des pokemon et du basket, pour qu'on puisse se marrer entre potes à la pause de temps en temps.

# Le développement

On voulait un peu un gameplay à la NBA 2K, mais en 2,5D, avec des pokémons. 
(Quand je parle de 2,5D, c'est en gros des sprites 2D, mais avec une caméra 3D, ce qui permet de faire des mouvements de caméra et des animations plus fluides et une certaine profondeur.)

Et donc on a taffé énormément autour des Scriptable Objects de Unity afin de pouvoir ajouter et retirer des pokemons comme bon nous semblait, c'était le truc le plus cool à la fin car rajouter des pokemons, devenait en soi un jeu à part entière pour nous.

# C'est tout ?

Et oui, c'est tout, c'est pour ça que c'est une petite note, pour se rappeler que ce projet a existé, il est dispo ici d'ailleurs : https://github.com/methil-mods/pokesket/tree/main 

J'ai pas beaucoup de visuels du jeu en développement, mais j'en ai mis quelques uns pour que vous puissiez voir à quoi ça ressemblait.

<div style="display: flex; flex-direction: column; gap: 20px; align-items: center; margin-top: 40px;">
  <img src="/pokesket_media/gif1.gif" alt="Pokesket Gameplay 1" style="width: 100%; max-width: 800px; border-radius: 8px;" />
  <img src="/pokesket_media/gif2.gif" alt="Pokesket Gameplay 2" style="width: 100%; max-width: 800px; border-radius: 8px;" />
  <img src="/pokesket_media/gif3.gif" alt="Pokesket Gameplay 3" style="width: 100%; max-width: 800px; border-radius: 8px;" />
</div>


