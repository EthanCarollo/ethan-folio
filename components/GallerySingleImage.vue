...
<template>
  <div class="w-full h-[100vh] swiper-no-swiping flex flex-col justify-center items-center">
    ...
    <div class ="lg:max-w-[65vw] lg:h-[50vh] max-w-[80vw] h-[60vw]">
      <div class ="gradient-border absolute right-0 top-0 w-full h-full"></div>
      <nuxt-img @mousedown.prevent :quality="20" :src="image" class="lg:max-w-[65vw] lg:h-[50vh] max-w-[80vw] h-[60vw] object-cover -translate-y-full"/>
    </div>
    <h1 class="lg:text-3xl text-xl text-center lg:w-[60vw] w-[80vw] mt-8">{{ info.title }}</h1>
    <h1 class="lg:text-xl text-sm text-center lg:w-[60vw] w-[80vw] mt-2">{{ info.description }}</h1>
  </div>
</template>
